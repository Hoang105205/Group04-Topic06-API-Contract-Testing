...
1. **Open/Create a Workspace**:
   ...
   - Select an existing workspace, or click **Create Workspace** to create a new one (e.g., named "EShop Testing").
...
2. Create a new Environment: Click the plus button (`+`) at the top of the left sidebar → select **Environment** → name it "EShop Env".
...
4. Select the "EShop Env" environment in the top-right corner dropdown of Postman.
![alt text](User_Guide_image/postman_selectenv.png)
5. Import Collection: Click the ellipsis button (`...`) in the left sidebar → select **Import** → select the `.json` collection file.
![alt text](User_Guide_image/postman_import.png)

### Verify Connection

Send `GET {{base_url}}/api/products` → receive JSON response → setup successful.
...
2. **POST `/api/auth/login`** — Login and receive the JWT authentication token.
...
The Collection Runner allows you to run your entire Postman Collection in a specified sequence. This is essential for testing the complete E-commerce integration flow (Login → Retrieve Products → Add to Cart → Place Order) to ensure variable chaining works seamlessly as an end-to-end integration test instead of isolated request executions.

1. **Open the Runner**: Right-click the **EShop API Collection** in the left sidebar and select **Run Collection** (or click the **Run** button at the top right of the Collection overview).
...
3. **Run the Collection**: Ensure the `"EShop Env"` environment is active, then click the **Run EShop API Collection** button.
...
- **Export Collection**: Click the ellipsis button (`...`) next to your Collection name → select **Export** → choose **Collection v2.1 (recommended)** → save as `EShop_API_Collection.json`.
- **Export Environment**: Select the **Environments** tab in the left sidebar → click the ellipsis button (`...`) next to `"EShop Env"` → select **Export** → save as `EShop_Environment.json`.
  ![alt text](User_Guide_image/postman_export_env.png)
...
1. **Installation**: Install Newman and its HTML reporter globally via npm:
   ```bash
   npm install -g newman
   npm install -g newman-reporter-html
   ```
2. **Running Collections**: Execute the following command to run your collection using the exported environment:
   ```bash
   newman run "EShop_API_Collection.json" -e "EShop_Environment.json"
   ```
3. **Generating Reports**: Newman supports multiple reporting formats to save your test results:
...
**Example Scenario**: We want to test the `POST /api/auth/login` endpoint with 5 different credentials combinations to verify both successful logins (200 OK) and negative validation cases (e.g., missing credentials or wrong password).
...
2. **Accessing Iteration Data in Test Scripts**:
   ...
   ```
3. **Running Iterations in Collection Runner**:
   - In the Collection Runner configuration, navigate to the **Data** section.
   - Click **Select File** and upload `login_data.csv`.
   - Postman will automatically set the **Iterations** count to match the number of rows (5 iterations) and run each row sequentially.
   ![alt text](User_Guide_image/postman_ddt_runner.png)

4. **Running Iterations in Newman**:
   Run the test suite using the `--iteration-data` (or shorthand `-d`) flag and specify the number of iterations:
   ```bash
